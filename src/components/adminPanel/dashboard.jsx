...
const AdminDashboard = ({ setAuthenticated }) => {
  ...
  const fetchAllProductsForSummary = async () => {
    ...
    try {
      // Assuming backend's /product without 'page' param now returns [data, total] for consistency
      // If it still returns only paginated data, you might need a dedicated /product/count endpoint
      const response = await axios.get('http://localhost:3000/product');
      const totalCount = Array.isArray(response.data) && response.data.length === 2 && typeof response.data[1] === 'number'
          ? response.data[1]
          : response.data.length; // Fallback if not returning tuple
  ...
  const fetchProductsByCategory = useCallback(async (pageToFetch = 1) => {
    if (selectedCategory) {
  ...
  // --- Handlers for Product Add Form (unchanged) ---
  ...
  const handleDeleteProduct = async (productId, productName) => {
    ...
    try {
      ...
      const newTotalPages = Math.ceil(newTotalProductsInCategory / productsPerPage);
      ...
      alert(error.response?.data?.message || `Failed to delete product "${productName}". Please try again.`);
    }
  };

  // --- NEW: Handle Product Card Click to Show Details ---
  const handleProductCardClick = async (productId) => {
    setLoadingProductDetail(true);
    setProductDetailError(null);
    setSelectedProductDetail(null); // Clear previous detail
    setShowProductDetailModal(true); // Open modal immediately

    try {
      const response = await axios.get(`http://localhost:3000/product/${productId}`);
      setSelectedProductDetail(response.data);
    } catch (err) {
      setProductDetailError('Failed to fetch product details. Please try again.');
  ...
  // --- Sidebar Navigation Handler (unchanged, with added resets for new states) ---
  ...
  // --- Render Content Based on Active Tab (unchanged) ---
      ...
      case 'dashboard':
        return (
          <div className="dashboard-summary">
            ...
            <p style={{ marginTop: '20px' }}>Use the sidebar to navigate to other sections.</p>
          </div>
        );
      case 'categories':
        return (
      ...
      <main className="main-content">
        {selectedCategory && activeTab === 'categories' ? (
          <div>
            ...
            <p>
              {selectedCategory.description} • {totalProductsInCategory} products
            </p>
            <button
            ...
            <div className="product-grid">
              {!loadingProducts && products.length > 0 && products.map((product) => (
                <div
                  ...
                  onClick={() => handleProductCardClick(product.id)} // <--- ATTACH CLICK HANDLER HERE
                >
                  <img
                    ...
                    alt={product.name}
                    className="product-image" // <--- ADDED CLASS FOR CSS TARGETING
                  />
                  <div className="product-details">
                    <h3>
                      {product.name}{' '}
                    ...
                    <div className="actions">
                      <button onClick={(e) => { e.stopPropagation(); alert('Edit coming soon!'); }}>Edit</button> {/* <--- STOP PROPAGATION */}
                      <button onClick={(e) => { e.stopPropagation(); handleDeleteProduct(product.id, product.name); }}>Delete</button> {/* <--- STOP PROPAGATION */}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <Dialog
              header={selectedProductDetail ? selectedProductDetail.name : "Product Details"}
              visible={showProductDetailModal}
              style={{ width: '60vw', maxWidth: '800px' }}
              onHide={() => {
                setShowProductDetailModal(false);
                setSelectedProductDetail(null); // Clear data when closing
                setProductDetailError(null);
              }}
              modal
            >
              {loadingProductDetail && <p>Loading product details...</p>}
              {productDetailError && <p style={{ color: 'red' }}>{productDetailError}</p>}
                ...
                <div className="product-detail-modal-content">
                  <div className="main-info">
                    ...
                    {selectedProductDetail.category && (
                      <p className="category-link">Category: {selectedProductDetail.category.name}</p>
                    )}
                  </div>
                  ...
                  {selectedProductDetail.images && selectedProductDetail.images.length === 0 && (
                      <p style={{ textAlign: 'center', color: '#888' }}>No images available for this product.</p>
                  )}
                </div>
              )}
            </Dialog>
